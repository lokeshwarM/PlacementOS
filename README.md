...
- **WhatsApp Provider Abstraction**: Pluggable `WhatsAppNotificationProvider` interface backed by a thread-safe `MockWhatsAppNotificationProvider` for testability with zero external network side-effects.
- **Reminder State & Stop-on-Done**: Recurring interval reminder engine that automatically stops active tasks and cancels pending outbox reminders when a student explicitly marks `APPLIED` or the deadline passes.
